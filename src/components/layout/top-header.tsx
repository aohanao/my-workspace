...
import { Calendar, Clock, Sparkles, Plus, Menu, Cloud, RefreshCw } from 'lucide-react'
...
export function TopHeader({ onOpenMobileMenu }: TopHeaderProps) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('workspace-sync-status', updateSync)
  }, [])
    ...
    <header className="h-14 sm:h-16 border-b border-white/[0.06] bg-[#090b10]/80 backdrop-blur-xl px-3.5 sm:px-6 flex items-center justify-between sticky top-0 z-20 shrink-0">
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        <div
          ...
          className="flex items-center gap-1 px-2 py-1 rounded-full bg-white/[0.02] border border-white/[0.06] text-[10px] text-zinc-400"
        >
          <span
            className={cn(
          ...
          <span className="hidden sm:inline">
            {syncStatus === 'synced' ? '云同步' : syncStatus === 'syncing' ? '同步中' : '本地'}
          </span>
        </div>
        ...
        <div className="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/[0.03] border border-white/[0.08] text-xs text-zinc-300">
          ...
          <span className="text-zinc-400 font-normal">论文盲审:</span>
          <span className="font-mono font-semibold text-white">
